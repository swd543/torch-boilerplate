...
class BrainSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        volumes = {}
        masks = {}
        for (dirpath, dirnames, filenames) in tqdm(os.walk(images_dir), postfix=f"reading {subset} images..."):
            image_slices = []
            mask_slices = []
        ...
        self.volumes = [(volumes[k], masks[k]) for k in tqdm(self.patients, desc=f"preprocessing {subset} volumes...")]

        # crop to smallest enclosing volume
        self.volumes = [crop_sample(v) for v in tqdm(self.volumes, desc=f"cropping {subset} volumes...")]

        # pad to square
        self.volumes = [pad_sample(v) for v in tqdm(self.volumes, desc=f"padding {subset} volumes...")]

        # resize
        self.volumes = [resize_sample(v, size=image_size) for v in tqdm(self.volumes, desc=f"resizing {subset} volumes...")]

        # normalize channel-wise
        self.volumes = [(normalize_volume(v), m) for v, m in tqdm(self.volumes, desc=f"normalizing {subset} volumes...")]
        ...
        self.volumes = [(v, m[..., np.newaxis]) for (v, m) in tqdm(self.volumes, desc=f"adding channel dim...")]
